...
//    connect(this, &TabBar::tabCloseRequested, this, [&](int index){
//        removeTab(index);
//        if (count() == 1)
...
//    }); // 关闭页面
    ...
    {
        emit plusClicked();
    }); // 添加页面将PushButton的时间传到TabWidget中
}
...
void TabBar::movePlusButton()
{
    ...
    {
        x += tabRect(i).width(); // x坐标为在所有tab后面
    }
    int h = this->geometry().top(); // y和tab一致
    int w = this->width(); // tab的总宽度
