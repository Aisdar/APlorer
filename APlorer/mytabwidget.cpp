...
{
    if (QString(this->childAt(event->pos())->parent()->metaObject()->className()) == "DriverWidget") {
        DriverWidget *driver = static_cast<DriverWidget*>(this->childAt(event->pos())->parent());
    ...
    //qDebug() << this->childAt(event->pos())->parent()->metaObject()->className();
}
